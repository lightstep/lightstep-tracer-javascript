...
.PHONY: release
release:
	@if [ $(shell git symbolic-ref --short -q HEAD) = "no_protobuf" ]; then exit 0; else \
		echo "Current git branch does not appear to be 'no_protobuf'. Refusing to publish."; exit 1; \
	fi
	npm run release:prepare $(RELEASE_TYPE)
	@echo
